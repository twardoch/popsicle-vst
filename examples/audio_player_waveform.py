...
class SimpleThumbnailComponent(juce_multi(juce.Component, juce.ChangeListener)):
    def __init__(self, sourceSamplesPerThumbnailSample, formatManager, cache):
        ...
        self.thumbnail = juce.AudioThumbnail(
            sourceSamplesPerThumbnailSample, formatManager, cache
        )
    ...
    def paintIfNoFileLoaded(self, g):
        ...
        g.drawFittedText(
            "No File Loaded", self.getLocalBounds(), juce.Justification.centred, 1
        )
    ...
    def paintIfFileLoaded(self, g):
        ...
        self.thumbnail.drawChannels(
            g, self.getLocalBounds(), 0.0, self.thumbnail.getTotalLength(), 1.0
        )
...
class MainContentComponent(
    juce_multi(cppyy.gbl.AudioAppComponent, juce.ChangeListener)
):
    ...
    def __init__(self):
        ...
        self.thumbnailComp = SimpleThumbnailComponent(
            512, self.formatManager, self.thumbnailCache
        )
    ...
    def resized(self):
        ...
        thumbnailBounds = juce.Rectangle[int](
            10, 100, self.getWidth() - 20, self.getHeight() - 120
        )
